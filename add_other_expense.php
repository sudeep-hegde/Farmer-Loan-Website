...
<html>
<head>
  <title>other Income details</title>
  <style>
body {
            margin: 0;
            font-family: "Mulish";                      
...
<div class="topnav">
    ...
    <form>
        <button type="submit" formaction="other_expense.php" style="margin-top:50px;margin-left:60px;height: 30px;width: 100px;
    ...
    <form method="post" action="add_other_expense.php"> 
    <fieldset> 
    ...
    <input type="text" name="description" placeholder="enter the description" style="width:100%;height:30px;
...
<?php
  ...
  $Query3="select count(*) from other_expense WHERE date='$date' and id='$id'";
  $Execute1 = mysqli_query($conn,$Query3);
  $count1 = mysqli_fetch_row($Execute1);
  if($count1[0]==0)
  {

    $sql = "INSERT INTO other_expense(id,date,amount,description)
    VALUES ('$id','$date','$amount','$desc')";
    if ($conn->query($sql) == TRUE) {
    echo'
    <div>
        <h1 style="color:#374151;font-size:20px; font-family: "Roboto", sans-serif;margin:auto;">New record for date: '
        .$date.' inserted successfully</h1>
    </div>';
    } else {
    ...
    // if($count[0]==1)
    // {
    //     $sql1 = "UPDATE expense SET other_expense='$amount' where date='$date' and id='$id'";
    ...
    // else if($count[0]==0)
    // {
    //     $sql1 = "INSERT INTO expense(id,date,other_expense)
