...
<div class="topnav">
    ...
    <form>
        <button type="submit" formaction="other_inc.php" style="margin-top:50px;margin-left:60px;height: 30px;width: 100px;cursor:pointer;border-radius:15px;
        border: 3px solid #3333ff;background-color:#2b3cbb;color:#f2f2f2;font-size:19px;font-family:Mulish">back</button>
    </form>

    <form method="post" action="add_other_inc.php"> 
    <fieldset> 
...
<?php
  ...
  $Query3="select count(*) from other WHERE date='$date' and id='$id'";
  $Execute1 = mysqli_query($conn,$Query3);
  $count1 = mysqli_fetch_row($Execute1);
  if($count1[0]==0)
  {

    $sql = "INSERT INTO other(id,date,amount,description)
    ...
    if($count[0]==1)
    {
        $sql1 = "UPDATE income SET other='$amount' where date='$date' and id='$id'";
    ...
    {
        $sql1 = "INSERT INTO income(id,date,other)
