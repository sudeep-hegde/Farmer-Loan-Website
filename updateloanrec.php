...
<div class="topnav">
    ...
    <form>
        ...
        border: 3px solid #3333ff;background-color:#2b3cbb;color:#f2f2f2;font-size:19px;font-family:Mulish" type="submit">back</button>
    </form>
    
    <form method="post" action="updateloanrec.php">
...
//echo "  INSERTION IN PROCESS";
  ...
  $Query2="select count(*) from loan_rec WHERE date='$date' and id='$id'";
  $Execute = mysqli_query($conn,$Query2);
  $count = mysqli_fetch_row($Execute);
  if($count[0]==1)
  {
    $sql = "UPDATE loan_rec SET amount_rec='$r_amount' ,amount_pending='$p_amount' ,desc1='$description' where date='$date' and id='$id'";
    if ($conn->multi_query($sql) == TRUE) 
    {
      echo'<div>
      <h1 style="color:#f2f2f2;font-size:20px; font-family: "Roboto", sans-serif;margin:auto;">
      for date: '. $date. ' income updated successfully</h1>
         </div>';
    } else {
      echo "Error: " . $sql . "<br>" . $conn->error;
    }

      $Query3="select count(*) from income WHERE date='$date' and id='$id'";
      $Execute1 = mysqli_query($conn,$Query3);
      $count1 = mysqli_fetch_row($Execute1);
      if($count1[0]==1){
        $sql1 = "UPDATE income SET loan_rec='$r_amount' where date='$date'";
        ...
        $sql1 = "insert into income(loan_rec,date) values('$r_amount','$date')";
      }
